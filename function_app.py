...
@app.schedule(schedule="0 30 1 * * *", arg_name="mytimer", run_on_startup=False,
              use_monitor=False)
@app.queue_output(arg_name="outputQueue", queue_name="pdf-compress-queue",
...
def EnqueuePDFs(mytimer: func.TimerRequest, outputQueue: func.Out[str]) -> None:
    ...
    for target in targets:
        ...
        if last_compressed:
            try:
                ...
                log(f"  Warning: Could not parse LastCompressed: {e}")
        ...
        try:
            ...
            log(f"  Getting drive ID for '{library_name}'...")
            ...
            target_summaries.append({"label": label, "site_url": site_url,
                                      "library_name": library_name, "count": target_count})
    ...
    file_log = "\n".join(file_log_lines + ["-" * 80, f"Total queued: {total_queued} files"])

    log("========================================")
    log(f">>> EnqueuePDFs COMPLETE - {total_queued} files queued, {skipped_count} skipped")
    log("========================================")
    ...
    try:
        ...
        graph.send_summary_email(
            ...
            subject=f"PDF Compressor - {now_utc.strftime('%Y-%m-%d')} - {total_queued} files queued",
            html_body=html,
            attachment_name=f"queued-files-{now_utc.strftime('%Y-%m-%d')}.txt",
...
@app.queue_trigger(arg_name="msg", queue_name="pdf-compress-queue",
                   connection="AzureWebJobsStorage")
        ...
        try:
            ...
            if list_id and list_item_id:
                try:
                    ...
                    log(f">>> Warning: metadata read failed (non-fatal): {e}")
